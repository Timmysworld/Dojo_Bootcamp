const pokémon = Object.freeze([
    ...
    const divIds =  pokémon.filter ( p => p.id % 3 === 0)
    ...
    const nameGreaterThan99 = pokémon.filter(p => p.id > 99).map(p=>p.name)
    const onlyPoisonType = pokémon.filter(p => p.types =="poison")
    const justTheFirstType = pokémon.filter(p => p.types[1] == "flying").map(p => p.name +  ':'  + p.types[0])
    const countNormalType = pokémon.filter(p => p.types == "normal").length
